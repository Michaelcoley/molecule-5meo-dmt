...
export async function exportPNG(viewer: MoleculeViewer, scale: number, transparent: boolean) {
  ...
  downloadBlob(blob, `${viewer.molecule.id}-${tag}.png`);
}
...
export function exportSVG(viewer: MoleculeViewer, labels: boolean, background: string | null) {
  ...
  downloadText(svg, `${viewer.molecule.id}-view.svg`, 'image/svg+xml');
}
...
export async function exportGLTF(viewer: MoleculeViewer, binary: boolean) {
  ...
  const id = viewer.molecule.id;
  if (binary) {
    downloadBlob(new Blob([result as ArrayBuffer], { type: 'model/gltf-binary' }), `${id}.glb`);
  } else {
    downloadText(JSON.stringify(result, null, 2), `${id}.gltf`, 'model/gltf+json');
  }
}
...
export function exportJSON(molecule: Molecule) {
  ...
  downloadText(JSON.stringify(data, null, 2), `${molecule.id}-data.json`, 'application/json');
}

export function exportSDF(molecule: Molecule) {
  downloadText(molecule.sdfText, `${molecule.id}.sdf`, 'chemical/x-mdl-sdfile');
}

export async function copySMILES(smiles: string): Promise<boolean> {
  try {
...
export function printInfoSheet(molecule: Molecule) {
  ...
  const comp = molecule.composition
    ...
    .join(' ');
  win.document.write(`<!doctype html><html><head><title>${meta.commonName} — Information Sheet</title>
  <style>
    @page { margin: 2cm; }
  ...
  <hr class="rule"/>
  <table>
    <tr><td>IUPAC name</td><td>${meta.iupacName}</td></tr>
    <tr><td>Molecular formula</td><td>${comp} &nbsp; (${meta.formula})</td></tr>
    <tr><td>Molecular weight</td><td>${meta.molecularWeight} g/mol</td></tr>
    <tr><td>PubChem CID</td><td>${meta.pubchemCID}</td></tr>
    <tr><td>InChIKey</td><td class="mono">${meta.inchiKey}</td></tr>
    <tr><td>Canonical SMILES</td><td class="mono">${meta.canonicalSMILES}</td></tr>
    <tr><td>Total atoms</td><td>${molecule.validation.counts.total}</td></tr>
  </table>
  <footer>${meta.conformerNote}<br/>Coordinates: PubChem CID ${meta.pubchemCID} 3D conformer.</footer>
